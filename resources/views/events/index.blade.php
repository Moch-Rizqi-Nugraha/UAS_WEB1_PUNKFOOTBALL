            ...
            <div class="bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow overflow-hidden h-full flex flex-col">
                ...
                @if($event->poster)
                    <div class="h-48 w-full overflow-hidden bg-gray-200">
                        <img src="{{ $event->getPosterUrl() }}" alt="{{ $event->name }}" class="w-full h-full object-cover">
                    </div>
                @else
                ...
                <div class="p-6 flex flex-col flex-grow">
                    ...
                    <div class="mb-3">
                        <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold text-white
                            @if($event->category === 'turnamen') bg-blue-500
                            @elseif($event->category === 'pelatihan') bg-green-500
                            @elseif($event->category === 'coaching') bg-purple-500
                            @else bg-gray-500 @endif">
                            {{ $event->getCategoryLabel() ?? ucfirst($event->category) }}
                        </span>
                    </div>
                    ...
                    <!-- Event Description -->
                    <p class="text-gray-600 text-sm mb-4 line-clamp-2">{{ $event->description ?? 'Event sepak bola' }}</p>
                    ...
                    <div class="space-y-2 mb-4 text-sm text-gray-500 flex-grow">
                        ...
                        <div class="flex items-start">
                            ...
                            <span class="truncate">{{ Str::limit($event->location, 25) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="mb-4 text-sm">
                        <div class="flex justify-between text-gray-600 mb-1">
                            <span>Peserta</span>
                            <span class="font-semibold">{{ $event->current_participants ?? 0 }}/{{ $event->max_participants ?? '-' }}</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-blue-500 h-2 rounded-full" style="width: {{ $event->max_participants ? (($event->current_participants ?? 0) / $event->max_participants * 100) : 0 }}%"></div>
                        </div>
                    </div>
                    
                    <!-- Register Button -->
                    <a href="{{ route('events.register', $event->id) }}" class="block w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white py-3 rounded-lg text-center font-bold hover:from-blue-700 hover:to-blue-800 transition mt-auto">
                        Daftar Sekarang
                    </a>
                </div>
            </div>
